...
public class freserva {
    ...
    public DefaultTableModel mostrar (String buscar){
        ...
        String [] titulos = {"ID","Id_habitacion","Numero","Idcliente","Cliente","Itrabajador","Trabajador","Tipo Reserva","Fecha Reserva","Fecha Ingreso","Fecha Salida","Costo","Estado"};
        ...
        sSQL="select r.idreserva,r.idhabitacion,h.numero,r.idcliente,"+
                ...
                " r.tipo_resrva,r.fecha_reserva,r.fecha_ingreso,r.fecha_salida,"+
        ...
        try {
            ...
            while(rs.next()){
                ...
                registro [7]=rs.getString("tipo_resrva");
    ...
    public boolean insertar(vreserva dts){
        sSQL="insert into reserva (idhabitacion,idcliente,idtrabajador,tipo_resrva,fecha_reserva,fecha_ingreso,fecha_salida,costo_alojamiento,estado)"+
                "values(?,?,?,?,?,?,?,?,?)";
        try {
     ...
     public boolean editar (vreserva dts){
         sSQL="update reserva set idhabitacion=?,idcliente=?,idtrabajador=?,tipo_resrva=?,fecha_reserva=?,fecha_ingreso=?,fecha_salida=?,costo_alojamiento=?,estado=?"+
                 "where idreserva=?";
     ...
     public boolean eliminar(vreserva dts){
      sSQL="delete from reserva whre idreserva=?";
          try {
